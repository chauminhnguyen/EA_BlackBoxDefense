...
parser.add_argument('--model_type', default='AE_DS', type=str,
                    ...
                    choices=['AE_DS', 'DS', 'only_denoiser'])
...
def main():
    ...
    if args.model_type == 'AE_DS':
        if args.pretrained_encoder:
            ...
            encoder = get_architecture(args.arch, args.dataset)
        ...
        if args.pretrained_decoder:
            ...
            decoder = get_architecture(args.arch, args.dataset)
...
def train(loader: DataLoader, denoiser: torch.nn.Module, criterion, optimizer: Optimizer, epoch: int, noise_sd: float,
    ...
    if args.optimization_method == 'ZO' and args.zo_method == 'GES':
        ...
        ges = GES(P, n, k, sur.surrogate_cls, std=0.1, alpha=0.5, beta=2, eta=10)
    ...
    for i, (inputs, targets) in enumerate(loader):
        ...
        if args.optimization_method == 'FO':
            ...
            if args.zo_method == 'RGE':
                ...
                # print(recon_flat.T.shape, grad_est_no_grad.shape)
                loss = torch.sum(recon_flat.T @ grad_est_no_grad, dim=-1).mean()
                # print(loss)
