...
from datasets import get_dataset, DATASETS, Origdataset
...
parser.add_argument('--model_type', default='AE_DS', type=str,
                    ...
                    choices=['AE_DS', 'DS', 'only_denoiser', 'AE'])
...
parser.add_argument('--train_method', default='whole', type=str,
                    ...
                    choices=['part', 'whole', 'whole_plus', "AE_only"])
...
def main():
    ...
    if args.model_type == 'AE_DS':
        if args.pretrained_encoder:
            ...
            # assert args.arch == args.encoder_arch
        ...
        if args.pretrained_decoder:
            ...
            # assert args.arch == args.decoder_arch
    ...
    if args.classifier != '':
        ...
        # clf = get_architecture(args.arch, args.dataset)
        num_classes = len(trainset.get_classes())
        clf = CNNModel(num_classes).cuda()
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        if args.train_objective == 'classification':
            ...
            if args.model_type == 'AE_DS':
                ...
                test_acc = -1 * (test_no_loss + test_no_loss_smooth + test_loss + test_loss_smooth + recon_loss + recon_loss_smooth + adv_loss + smooth_loss)
...
# def train_from_scratch(loader: DataLoader, encoder: torch.nn.Module, decoder: torch.nn.Module, denoiser: torch.nn.Module, criterion,
...
def train(loader: DataLoader, denoiser: torch.nn.Module, criterion, optimizer: Optimizer, epoch: int, noise_sd: float,
    ...
    if args.optimization_method == 'ZO' and args.zo_method == 'GES':
        k = args.q
        P = k
        # n = 3*64*64
        ...
        # ges = GES(P, n, k, sur.surrogate_cls, std=1, alpha=0.5, beta=2, eta=1)
        ges = GES(P, n, k, sur.surrogate_cls, std=0.1, alpha=0.5, beta=1, eta=0.1)
    ...
    for i, (inputs, targets) in enumerate(loader):
        ...
        if args.optimization_method == 'FO':
            ...
            if args.zo_method == 'RGE':
                ...
                grad_est_no_grad = grad_est_no_grad / (grad_est_no_grad.norm() + 1e-8)
                recon_flat = recon.view(batch_size, -1)
                loss = (recon_flat.T @ grad_est_no_grad).mean()
        ...
        if args.optimization_method == 'FO':
            ...
            if args.zo_method =='RGE':
                with torch.no_grad():
                    ...
                    # print(recon_pre)
                    # print(recon_pre.shape, original_pre.shape)
