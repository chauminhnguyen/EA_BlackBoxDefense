...
class AddTinyImageNetTrigger(AddTrigger):
    """Add WaNet trigger to CIFAR10 image.
    ...
    def __init__(self, identity_grid, noise_grid, noise=False, s=0.5, grid_rescale=1, noise_rescale=2):
        super(AddTinyImageNetTrigger, self).__init__()
...
class PoisonedCelebA(CelebA):
    def __init__(self,
                 ...
                 poisoned_transform_index,
                 poisoned_target_transform_index):
        super(PoisonedCelebA, self).__init__(
            benign_dataset.root,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
            self.poisoned_transform_noise = copy.deepcopy(self.transform) # add noise
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddTinyImageNetTrigger(identity_grid, noise_grid,  noise=False))
        #add noise transform
        self.poisoned_transform_noise.transforms.insert(poisoned_transform_index, AddTinyImageNetTrigger(identity_grid, noise_grid,  noise=True))
    ...
    def __getitem__(self, index):
        ...
        # TODO: Targets are not determined
        target = []
...
class PoisonedDataset(Origdataset):
    def __init__(self,
                 benign_dataset,
                 y_target,
                 poisoned_rate,
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, identity_grid, noise_grid, noise, poisoned_transform_index, poisoned_target_transform_index):
    ...
    if class_name == DatasetFolder:
        ...
        return PoisonedCelebA(benign_dataset, y_target, poisoned_rate, identity_grid, noise_grid, noise, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == Origdataset:
        return PoisonedDataset(benign_dataset, y_target, poisoned_rate, identity_grid, noise_grid, noise, poisoned_transform_index, poisoned_target_transform_index)
    else:
