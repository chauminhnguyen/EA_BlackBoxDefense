...
class ADDTinyImageNetTrigger(AddTrigger):
    ...
    def __init__(self, pattern, weight):
        ...
        if pattern is None:
            self.pattern = torch.zeros((1, 64, 64), dtype=torch.uint8)
            self.pattern[0, -3:, -3:] = 255
        else:
            self.pattern = pattern
            if self.pattern.dim() == 2:
        ...
        if weight is None:
            self.weight = torch.zeros((1, 64, 64), dtype=torch.float32)
            self.weight[0, -3:, -3:] = 1.0
        else:
            self.weight = weight
            if self.weight.dim() == 2:
...
class PoisonedCIFAR10(CIFAR10):
    ...
    def __getitem__(self, index):
        ori, target = self.data[index], int(self.targets[index])
...
class PoisonedCelebA(CelebA):
    def __init__(self, benign_dataset,
                 ...
                 poisoned_transform_index,
                 poisoned_target_transform_index):
        super(PoisonedCelebA, self).__init__(benign_dataset.root, split=benign_dataset.split, target_type="identity", transform= benign_dataset.transform,
    ...
    def __getitem__(self, index):
        ...
        if index in self.poisoned_set:
            ...
            # TODO: Targets are not determined
            target = self.poisoned_target_transform(target[0])
            ...
            if self.target_transform is not None:
                # TODO: Targets are not determined
                target = self.target_transform(target[0].item())
...
class PoisonedDataset(Origdataset):
    ...
    def __init__(self,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        self.poisoned_transform.transforms.insert(poisoned_transform_index, ADDTinyImageNetTrigger(pattern, weight))
